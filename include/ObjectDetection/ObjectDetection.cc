...
void ObjectDetection::run_object_detection(cv::Mat &image) {
  ...
  float scale = std::min(input_dimensions_.width / (image.cols*1.0), input_dimensions_.height / (image.rows*1.0));  // TODO(simon) Magic number.
...
cv::Mat ObjectDetection::static_resize(cv::Mat &img) {
  float r = std::min(input_dimensions_.width / (img.cols*1.0), input_dimensions_.height / (img.rows*1.0));  // TODO(simon) Magic number.
  // r = std::min(r, 1.0f);
  ...
  cv::Mat out(input_dimensions_.height, input_dimensions_.width, CV_8UC3, cv::Scalar(114, 114, 114));  // TODO(simon) Magic number.
...
void ObjectDetection::decode_outputs(const float *prob,
  ...
  if (proposals.size()>0){
    qsort_descent_inplace(proposals, 0, 0); // TODO(simon) Getting: "Process finished with exit code 139 (interrupted by signal 11: SIGSEGV)"  // TODO(simon) Magic number.
  }
  ...
  for (int i = 0; i < count; i++)  // TODO(simon) Magic number.
  {
    ...
    // clip
    x0 = std::max(std::min(x0, (float)(img_w - 1)), 0.f);  // TODO(simon) Magic number.
    y0 = std::max(std::min(y0, (float)(img_h - 1)), 0.f);  // TODO(simon) Magic number.
    x1 = std::max(std::min(x1, (float)(img_w - 1)), 0.f);  // TODO(simon) Magic number.
    y1 = std::max(std::min(y1, (float)(img_h - 1)), 0.f);  // TODO(simon) Magic number.
...
void ObjectDetection::nms_sorted_bboxes(const std::vector<Object> &faceobjects,
  ...
  for (int i = 0; i < n; i++)  // TODO(simon) Magic number.
  {
    ...
    for (int j = 0; j < (int)picked.size(); j++)  // TODO(simon) Magic number.
    {
...
void ObjectDetection::draw_objects(const cv::Mat &bgr, const std::vector<Object> &objects) {
  ...
  static const char* class_names[] = {  // TODO(simon) Class names should be set in the configuration.
      "pallet"
  };
...
//            obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
    ...
    sprintf(text, "%s %.1f%%", class_names[obj.label], obj.prob * 100);  // TODO(simon) Magic number.

    int baseLine = 0;  // TODO(simon) Magic number.
    cv::Size label_size = cv::getTextSize(text, cv::FONT_HERSHEY_SIMPLEX, 0.4, 1, &baseLine);  // TODO(simon) Magic number.
    ...
    cv::rectangle(bgr, cv::Rect(cv::Point(x, y), cv::Size(label_size.width, label_size.height + baseLine)),
                  txt_bk_color, -1);  // TODO(simon) Magic number.
    // obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height
...
object_detection_output ObjectDetection::get_detection() {
  ...
  uint32_t image_width = 1280; // TODO(simon) Set this as an input image_.cols  // TODO(simon) Magic number.
  uint32_t image_height = 720; // TODO(simon) Set this as an input image_.rows  // TODO(simon) Magic number.
...
double ObjectDetection::box_filtering(double image_width, //TODO(simon) Filtering of selecting the middle most down box
  ...
  if (bbox_center_bottom.at(0)>image_width/2){//! Right side of image  // TODO(simon) Magic number.
    width_fraction_score = 1-((bbox_center_bottom.at(0)-(image_width/2))/(image_width/2));  // TODO(simon) Magic number.
  }
  else{//! Left side of image
    width_fraction_score = bbox_center_bottom.at(0)/(image_width/2);  // TODO(simon) Magic number.
  }
